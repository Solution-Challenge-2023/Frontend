...
class _ScanScreenState extends State<ScanScreen> {
  ...
  void _processOcrResult(String ocrText) {
    ...
    // 위젯을 업데이트하여 상품 목록을 표시
    setState(() {
      // 짝수 인덱스 행에는 상품 이름이, 홀수 인덱스 행에는 상품 수량이 있음
      for (int i = 0; i < lines.length; i++) {
        if (i % 2 == 0) {
          productNames.add(lines[i]);
        } else {
          productCounts.add(int.tryParse(lines[i]) ?? 1);
        }
      }

      products.clear(); // 기존 상품 목록을 지우고 새로 업데이트
      for (int i = 0; i < productNames.length; i++) {
        Product product = Product(
          ...
          expiredType: ExpiredType.expiryDate, // expiredType은 임시로 expiryDate로 설정
        );
          ...
          ElevatedButton(
            onPressed: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ItemList(products: products),
                ),
              );
            },
            child: Text('Add'),
