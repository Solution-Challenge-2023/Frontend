...
class _ScanScreenState extends State<ScanScreen> {
  ...
  Future<void> _scanImage() async {
    ...
    final RecognisedText recognisedText = await textDetector.processImage(
        inputImage);
    String ocrText = recognisedText.text;
    setState(() {
  ...
  void _processOcrResult(String ocrText) {
    // 상품명 추출
    RegExp productNameRegex = RegExp(r'상품명\s*(\S+)');
    Match? productNameMatch = productNameRegex.firstMatch(ocrText);
    if (productNameMatch != null) {
      _productName = productNameMatch.group(1) ?? '';
    }

    // 상품 개수 추출
    RegExp productCountRegex = RegExp(r'수량\s*(\d+)');
    Match? productCountMatch = productCountRegex.firstMatch(ocrText);
    if (productCountMatch != null) {
      _productCount = int.tryParse(productCountMatch.group(1) ?? '') ?? 0;
    }
  }
    ...
    return Scaffold(
      ...
      Column(
        children: <Widget>[
          ...
          ElevatedButton(
            onPressed: () {
              // 상품명과 상품 개수를 사용하여 Product 객체 생성
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ItemList(products: [product]),
                ),
              );
            },
            child: Text('상품 추가'),
